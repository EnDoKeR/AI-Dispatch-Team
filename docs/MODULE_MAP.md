...
| `scripts/run_private_ratecon_measurement.py` | scripts | private RateCon local audit | local_only | CLI with explicit confirmation | manual private audit only | measurement orchestration, report/audit writers, review exports, Google sync | keep shrinking | Public CLI wrapper. Parser/config/safety now live under `app/document_ai/measurement_cli/`; do not add new report/audit logic here. |
