...
* [Document Classification Calibration](docs/DOCUMENT_CLASSIFICATION_CALIBRATION.md)
* [Layout-Aware Digital Extraction](docs/LAYOUT_AWARE_DIGITAL_EXTRACTION.md)
* [Layout Extraction Dependency Review](docs/LAYOUT_EXTRACTION_DEPENDENCY_REVIEW.md)
* [Layout Provider Decision](docs/LAYOUT_PROVIDER_DECISION.md)
* [Layout Evidence Fusion and Stop Association](docs/LAYOUT_EVIDENCE_FUSION_AND_STOP_ASSOCIATION.md)
* [Layout Stop Normalization and Evaluation](docs/LAYOUT_STOP_NORMALIZATION_AND_EVALUATION.md)
* [Stop Normalization Debug and Calibration](docs/STOP_NORMALIZATION_DEBUG_AND_CALIBRATION.md)
* [Stop Group Provenance Audit](docs/STOP_GROUP_PROVENANCE_AUDIT.md)
...
The current official RateCon architecture is documented in `docs/RATECON_PIPELINE_CURRENT_STATE.md`: PDF triage, safe extraction artifacts, fake/anonymized text artifacts, document/page/section classification, extraction scope filtering, layout artifacts, the `pdfplumber` provider pilot, layout-aware candidates, optional layout/text fusion with no-regression guardrails, normalized stops, generic candidates, broker template matching, template-aware scoring, conservative resolution, RateConfirmationIntake drafts, and validation. Classification calibration is documented in `docs/DOCUMENT_CLASSIFICATION_CALIBRATION.md`; safe measurement separates extraction-relevant documents, normal load movement documents, TONU/payment confirmations, supplemental-only documents, unknown-review documents, and OCR-needed documents. Layout-aware extraction and provider selection are documented in `docs/LAYOUT_AWARE_DIGITAL_EXTRACTION.md` and `docs/LAYOUT_PROVIDER_DECISION.md`; opt-in candidate fusion, stop association, provider diagnostics, table-profile comparison, stop-signal issue buckets, normalized stop sets, and local-only stop review packets are documented in `docs/LAYOUT_EVIDENCE_FUSION_AND_STOP_ASSOCIATION.md` and `docs/LAYOUT_STOP_NORMALIZATION_AND_EVALUATION.md`. Stop calibration and local-only review rules are documented in `docs/STOP_NORMALIZATION_DEBUG_AND_CALIBRATION.md`; current safe measurement shows the next block is deeper stop grouping/merge hardening, while OCR, Vision, and Camelot remain decision-gated. Hard-layout resolver behavior is documented in `docs/RATECON_TEMPLATE_RESOLVER_HARDENING.md`. Private broker template overlay workflow is documented in `docs/PRIVATE_BROKER_TEMPLATE_OVERLAY.md`; private templates must remain local/ignored and safe summaries use aliases only. The old direct RateCon PDF/regex prototypes were removed; see `docs/archive/LEGACY_RATECON_REGEX_PROTOTYPES.md`.
...
Use `py scripts/run_tests.py` as the official full-suite command. It fails if
test discovery returns zero tests.
