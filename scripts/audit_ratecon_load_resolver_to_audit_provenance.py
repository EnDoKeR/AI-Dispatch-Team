"""Static RateCon load resolver-to-audit provenance audit.
...
DEFAULT_OUTPUT_DIR = Path(".local_outputs/ratecon_load_resolver_to_audit_provenance_audit")
...
TEXT_MARKERS = (
    "resolver_to_audit",
    "load_resolver_to_audit",
    "resolver_visible",
    "load_resolver_visible_candidates",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Static RateCon load resolver-to-audit provenance audit."
    )
...
def _owner_recommendation(rel: str) -> str:
    if rel == "app/document_ai/load_identifier_resolver_to_audit_provenance.py":
        return "resolver_to_audit_sidecar_owner"
    if rel == "app/document_ai/load_identifier_generated_provenance_boundary.py":
        return "boundary_compare_consumer"
    if rel == "app/document_ai/load_identifier_generated_resolver_provenance.py":
        return "generated_resolver_sidecar_consumer"
...
def _risk_rows(modules: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in modules:
        ...
        if rel.startswith("app/") and _owner_recommendation(rel) == "manual_review_required":
            risks.append(
                {
                    "module_path": rel,
                    "risk": "manual_review_required_for_resolver_to_audit_logic",
                    "reason": "Resolver-to-audit provenance markers appear outside approved local-only owners.",
                }
            )
    return risks
...
def _report(summary: dict[str, Any]) -> str:
    lines = [
        "# RateCon Load Resolver-to-Audit Provenance Audit",
        "",
...
def build_audit(repo_root: Path) -> dict[str, Any]:
    ...
    summary = {
        "schema_version": "ratecon_load_resolver_to_audit_provenance_audit_v1",
...
def write_outputs(output_dir: Path, payload: dict[str, Any]) -> dict[str, Path]:
    ...
    paths = {
        ...
        "modules": output_dir / "load_resolver_to_audit_modules.csv",
        "symbols": output_dir / "load_resolver_to_audit_symbols.csv",
        "field_map": output_dir / "load_resolver_to_audit_field_map.csv",
        "risk_findings": output_dir / "load_resolver_to_audit_risk_findings.csv",
    }
    _write_json(paths["summary"], payload["summary"])
    paths["report"].write_text(_report(payload["summary"]), encoding="utf-8")
    _write_csv(paths["modules"], payload["modules"], ["module_path", "marker_count", "markers", "owner_recommendation"])
    _write_csv(paths["symbols"], payload["symbols"], ["module_path", "symbol_name", "line", "symbol_kind", "owner_recommendation"])
    _write_csv(paths["field_map"], payload["fields"], ["module_path", "field_name", "reference_count", "owner_recommendation"])
    _write_csv(paths["risk_findings"], payload["risks"], ["module_path", "risk", "reason"])
    return paths
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("RateCon load resolver-to-audit provenance audit")
