...
from app.document_ai.rate_conflict_audit import (
    ...
    RATE_CONFLICT_AUDIT_RAW_JSON,
)
from scripts import analyze_rate_conflicts as cli
...
class AnalyzeRateConflictsCliTests(unittest.TestCase):
    def test_cli_writes_reports_and_console_is_safe(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(RATE_AUDIT_MULTIPLE_DIFFERENT_STRONG_TOTALS, output)
            self.assertIn("money_values_printed: False", output)
            self.assertNotIn("$", output)
            self.assertNotIn("FAKE_RATE", output)
            self.assertTrue((root / RATE_CONFLICT_AUDIT_MD).exists())
            self.assertTrue((root / RATE_CONFLICT_AUDIT_JSON).exists())

    def test_cli_prefers_raw_artifact(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / RATE_CONFLICT_AUDIT_RAW_JSON).write_text(
                json.dumps(
                    {
                        "records": [
                            {
            ...
            self.assertEqual(result, 0)
            self.assertIn("equivalent_group_count: 1", stdout.getvalue())
    ...
    def test_cli_missing_data_returns_friendly_error(self):
        ...
        self.assertIn("rate_conflict_audit_error", stdout.getvalue())
