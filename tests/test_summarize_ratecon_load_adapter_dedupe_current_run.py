...
SCRIPT = ROOT / "scripts" / "summarize_ratecon_load_adapter_dedupe_current_run.py"
FIXTURES = ROOT / "tests" / "fixtures" / "ratecon_load_adapter_dedupe_current_run"


class SummarizeRateconLoadAdapterDedupeCurrentRunTests(unittest.TestCase):
    ...
    def _run_fixture(self, tmp_path: Path, fixture: str) -> subprocess.CompletedProcess:
        ...
        command = [
            ...
            "--boundary-compare-dir",
            str(fixture_dir / "boundary_compare"),
            "--output-dir",
    ...
    def _summary(self, tmp_path: Path, fixture: str) -> dict:
        return json.loads(
            (
                ...
                / "load_adapter_dedupe_current_run_summary.json"
    ...
    def test_current_run_status_fixtures(self):
        fixtures = (
            ...
            "generation_to_adapter_loss",
            "adapter_to_dedupe_loss",
            "dedupe_to_resolver_loss",
            "resolver_to_audit_loss",
            "audit_to_evaluator_loss",
    ...
    def test_outputs_are_written_and_reports_redact_values(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            outputs = {
                ...
                for filename in (
                    "load_adapter_dedupe_current_run_summary.json",
                    "load_adapter_dedupe_current_run_report.md",
                    "load_adapter_dedupe_current_run_gate.csv",
                    "load_adapter_dedupe_current_run_next_actions.csv",
                    "load_adapter_dedupe_current_run_known_debt.csv",
                )
            }
            report = (output_dir / "load_adapter_dedupe_current_run_report.md").read_text(
                encoding="utf-8"
            )
