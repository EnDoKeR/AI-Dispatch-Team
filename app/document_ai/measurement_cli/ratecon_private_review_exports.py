"""Local-only review/export artifact helpers for private RateCon measurement."""
...
def normalized_stop_sets_from_measurement_rows(rows):
    return [
        row.get("normalized_stop_set")
        for row in rows or []
        if isinstance(row, dict) and row.get("normalized_stop_set")
    ]
...
def write_private_ratecon_review_packet_exports(
    measurement_rows,
    output_dir=None,
    include_private_values_local_only=False,
):
    stop_sets = normalized_stop_sets_from_measurement_rows(measurement_rows)
    return write_stop_review_packet(
        stop_sets,
...
def private_ratecon_review_packet_export_labels(packet):
    return {
        "csv": output_file_name(packet["csv"]),
        "md": output_file_name(packet["md"]),
        "row_count": packet["row_count"],
        "include_private_values_local_only": packet[
            "include_private_values_local_only"
        ],
    }


def private_ratecon_review_export_labels(export):
    labels = {
...
def write_private_ratecon_value_review_exports(
    rows,
    output_dir=None,
    allow_custom_output_dir=False,
):
    return write_private_ratecon_value_review_template_csv(
        rows,
        output_dir=output_dir,
...
def write_private_ratecon_review_exports(
    measurement_rows,
    output_dir=None,
    include_private_values_local_only=False,
):
    return {
        "stop_review_packet": write_private_ratecon_review_packet_exports(
            measurement_rows,
            output_dir=output_dir,
            include_private_values_local_only=include_private_values_local_only,
        )
